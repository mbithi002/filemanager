...
import { Toaster, VotingForm } from '../components/components';
...
function Home() {
  ...
  const { getUserVoteStatus } = useVotingData()
  ...
  return (
    <>
      {status && !voteStatus && (
        voteForm && (
          <div className="fixed z-50 sm:px-0 px-2 w-[100vw] top-2 flex justify-center items-center min-h-screen bg-black bg-opacity-10">
            <div className="flex flex-col w-full items-center">
              <div onClick={() => setVoteForm(false)} className="cursor-pointer">
                <i className='fa-solid fa-circle-xmark text-2xl text-white z-50'></i>
              </div>
              <VotingForm name={userData.name} email={userData.email} />
            </div>
          </div>
        )
      )}
        ...
        <div className="container my-10">
          <div className="grid sm:grid-cols-2 min-h-[70dvh] p-3 gap-5">
            <div className="flex flex-col justify-between">
              ...
              <div className="self-start flex flex-row sm:-mt-10 sm:mb-10">
                <Link to={'/dashboard'}>
                ...
                {status && (
                  voteStatus ? (
                    <Link to={'/votes'}>
                      <button className='p-2 m-2 rounded-[3rem] bg-teal-400 text-white w-[7rem] bounce-top bounce-top'>Votes</button>
                    </Link>
                  ) : (
                    <button onClick={() => setVoteForm(true)} className='p-2 m-2 rounded-[3rem] bg-teal-400 w-[7rem] bounce-top bounce-top'>Vote</button>
                  )
                )}
              </div>
            </div>
